	...
	<div class="bbs-category clearfix">
		...
		<form name="frm" id="frm" action="search.no" method="get">
			<div class="bbs-sch">
				...
				<input type="hidden" name="boardCode" value="${n.boardCode}">
				...
				<button type="submit" class="findBtn" id="findBtn">검색</button>
			</div>
		</form>
		...
		<div class="paginationSet">
			...
			<c:choose>
				<c:when test="${pi.currentPage ne 1}">
					<li class="i prev">
						<a title="이전 목록" href="list.no?boardCode=${boardCode}&currentPage=${pi.currentPage-1 }"> 
							<span>이전 </span> 
						</a>
					</li>
				</c:when>
			...
			<c:forEach begin="${pi.startPage}" end="${pi.endPage}" var="p">
				<c:choose>
					...
					<c:otherwise>
						<li class="active"><a href="list.no?boardCode=${boardCode}&currentPage=${p}"><span>${p}</span></a></li>
					</c:otherwise>
				</c:choose>
			</c:forEach>
			<c:choose>
				...
				<c:otherwise>
					<li class="i next">
						<a title="다음 목록" href="list.no?boardCode=${boardCode}&currentPage=${pi.currentPage+1}">
							<span>다음 </span>
						</a> 
					</li>
				</c:otherwise>
